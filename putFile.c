...
void putCarcInRom(FILE *Carc, FILE *Mkds, int Id, char isTex)
{
    ...
    getFat(Mkds);
    // printf("%x\n", allTheFat[CROSSnDL + 3].addressStart);

    ENTRY_FAT course;
    if (isTex)
        course = getCourseTex(Id);

    else
        course = getCourse(Id);

    putFileInRom(Carc, Mkds, course);

    /* unsigned int sizeReplData = course.size;
    // sizeReplData = 0;

    void *restOfMKDS = getRestOfFileWithOffsetAlsoSetCursorToAddrOfReplacedData(Mkds, course.addressStart, sizeReplData);

    int sizeOfRestMKDS = _getRelativeFileLength(Mkds, ftell(Mkds)) - sizeReplData;

    fwrite(pCarc, sizeCarc, 1, Mkds);

    fwrite(restOfMKDS, sizeOfRestMKDS, 1, Mkds);

    moveAddrRelatToStartAddr(course, sizeCarc, Mkds);

    _fileSizeChange(Mkds, sizeCarc - sizeReplData);
    ...
    // COPIED
    unsigned short CRC = _COPIEDGetCRC16(Header, 0x15E);
    free(Header);
    //-
...
{
    unsigned char size;
    fseek(Mkds, 0x14, SEEK_SET);
    fread(&size, 1, 1, Mkds);

    unsigned int sizeTot = getFileLength(Mkds);

    // printf("%d  %d  %d\n", sizeTot, (int)pow((double)2, (double)(20 + size)), size);

    while (sizeTot * 8 > ((int)pow((double)2, (double)(20 + size))))
    {
        size = size + 1;
    }
    fseek(Mkds, 0x14, SEEK_SET);

    // printf("%d  %d   %d  %x\n", sizeTot, (int)pow((double)2, (double)(20 + size)), size, ftell(Mkds));

    fwrite(&size, 1, 1, Mkds);
    fseek(Mkds, 0x80, SEEK_SET);
    unsigned int sizing2;
    fread(&sizing2, 4, 1, Mkds);
...
// COPIED FROM EFE, SO FROM GERICOM (I DO NOT UNDERSTAND LOL)
...
unsigned short _COPIEDGetCRC16(unsigned char *Data, unsigned int size)
{
    unsigned short result = 0xFFFF;
    for (int i = 0; i < size; i++)
    {
        result = (unsigned short)((result >> 8) ^ CRC16Table[(result ^ Data[i]) & 0xFF]);
    }
